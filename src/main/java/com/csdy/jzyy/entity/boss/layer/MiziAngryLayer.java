package com.csdy.jzyy.entity.boss.layer;
...
@OnlyIn(Dist.CLIENT)
public class MiziAngryLayer extends RenderLayer {
    //血凯
    ...
    private final HumanoidModel<MiziAo> model;
    private final EntityRenderer render;

    public MiziAngryLayer(EntityRenderer renderer) {
    ...
    protected EntityModel<MiziAo> model() {
        return this.model;
    }
    ...
    public void render(PoseStack poseStack, MultiBufferSource buffer, int packedLight,
        ...
        if (!(entity instanceof MiziAo miziAo)) return;
        ...
        // 2. 稍微放大模型使其显示在盔甲外面
        float scale = 1.5f; // 2%放大
        poseStack.scale(scale, scale, scale);

        // 3. 调整渲染参数
        float time = (float)miziAo.tickCount + partialTicks;
        ...
        // 4. 设置模型动画并渲染
        this.model.prepareMobModel(miziAo, limbSwing, limbSwingAmount, partialTicks);
        this.getParentModel().copyPropertiesTo(this.model);
        this.model.setupAnim(miziAo, limbSwing, limbSwingAmount, ageInTicks, netHeadYaw, headPitch);
        this.model.renderToBuffer(
                poseStack,
                vertexConsumer,
                packedLight,
                OverlayTexture.NO_OVERLAY,
                1F, 0F, 0F, 1.0F // 调整颜色和透明度
        );
