...
public class BladeRelease3 extends NoLevelsModifier implements RequirementsModifierHook, ValidateModifierHook, ToolStatsModifierHook {

    @Override
    public @org.jetbrains.annotations.Nullable Component validate(IToolStackView tool, ModifierEntry entry) {
        if (tool.getModifierLevel(ModifierRegister.BLADE_RELEASE2_STATIC_MODIFIER.getId()) > 0) return null;
    ...
    public Component requirementsError(ModifierEntry entry) {
        return Component.translatable("modifier.jzyy.blade_release3.requirements") ;
    }

    @Override
    public void addToolStats(IToolContext context, ModifierEntry entry, ModifierStatsBuilder builder) {
        ATTACK_DAMAGE.add(builder, 8);
    }
