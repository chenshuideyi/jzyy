...
public class FakeKillAuraClientDiadema extends ClientDiadema {
    ...
    private void drawParticle(Level level) {
        PointSets.Circle(6,80).map(vec3 -> vec3.add(getPosition())).forEach(vec3 -> level.addParticle(type, vec3.x,vec3.y,vec3.z,0,0,0));
