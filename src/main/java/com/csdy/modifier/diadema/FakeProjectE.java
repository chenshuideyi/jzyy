...
public class FakeProjectE extends DiademaModifier implements EquipmentChangeModifierHook {
    private Diadema fakeprojecte;
    @Override
        ...
        if (entity instanceof Player player) {
            if (context.getLevel() instanceof ServerLevel serverLevel) {
                if (context.getChangedSlot().isArmor()) {
                    this.fakeprojecte = DiademaRegister.FAKE_PROJECTE.get().CreateInstance(new FollowDiademaMovement(player));
                }
            }
        }
    }
