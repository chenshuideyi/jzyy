...
public class Luxuria extends DiademaModifier implements EquipmentChangeModifierHook {
    private Diadema luxuria;
    @Override
        ...
        if (entity instanceof Player player) {
            if (context.getLevel() instanceof ServerLevel serverLevel) {
                if (context.getChangedSlot().isArmor()) {
                    this.luxuria = DiademaRegister.LUXURIA.get().CreateInstance(new FollowDiademaMovement(player));
                }
            }
        }
    }
    ...
    public void onUnequip(IToolStackView tool, ModifierEntry modifier, EquipmentChangeContext context) {
        ...
        if (entity instanceof Player) {
            if (context.getChangedSlot().isArmor() && luxuria!=null) {
                this.luxuria.kill();
            }
        }
    }
